...
namespace VisiLibity
{
    ...
    public:
        ...
        bool is_endpoint_of(const Line_Segment &line_segment_temp,
                            double epsilon = 0.0) const;
        /** \brief  calculates the ccw oriented angle offset from the segment ab to the segment bc
         *
         * \author Derek Kingston
         * \remark Points a,b,c must be separated by epsilon > 0
         */
        static Angle angle_separation(const Point &a, const Point &b, const Point &c);
        /** \brief  calculates the point p rotated ccw around the origin by th
         *
         * \author Derek Kingston
         */
        static Point rotate(const Point &p, double th);
        /** \brief  returns a point in the same direction as the point p
         *          but with length 1.0
         *
         * \author Derek Kingston
         */
    ...
    public:
        ...
        Line_Segment(const Point &pointemp);
        /// Line_Segment pointing from first_pointemp to second_pointemp
        Line_Segment(const Point &first_pointemp,
                     const Point &second_pointemp, double epsilon = 0);
        // Accessors
         ...
         *
         * \remarks  if \a pointemp is w/in a distance \a epsilon of an existing
         * endpoint, the coordinates of \a pointemp are used and size is set to
         * 1 as appropriate
         */
        void set_first(const Point &pointemp, double epsilon = 0.0);
         ...
         * 1 as appropriate
         */
        void set_second(const Point &pointemp, double epsilon = 0.0);
    ...
    /** \brief  intersection of Line_Segments treated as infinitely long lines
     *
     * \author  Derek Kingston
     * \return a Line_Segment of size 0, 1, or 2
     * \remarks  size 0 results if the lines are parallel or either line segment
     * has size less than 2. size 1 results otherwise. note, numerically unstable
     * when the lines are nearly parallel
     */
    Line_Segment intersection_infinite(const Line_Segment &line_segment1,
                                       const Line_Segment &line_segment2,
                                       double epsilon = 0.0);
    ...
    public:
         ...
         *
         * \pre  member data of \a polar_origin_temp and \a pointemp have
         * been assigned (numbers)
         * \param polar_origin_temp  the origin of the polar coordinate system
         * \param pointemp  the point to be represented
         * \remarks  if polar_origin_temp == pointemp, the default
    ...
    public:
         ...
         */
        Ray() {}
        /// construct ray emanating from \a base_pointemp in the direction
        /// \a bearing_temp
        Ray(Point base_pointemp, Angle bearing_temp) : base_point_(base_pointemp), bearing_(bearing_temp) {}
        /// construct ray emanating from \a base_pointemp towards
        /// \a bearing_point
        Ray(Point base_pointemp, Point bearing_point);
        // Accessors
        /// get base point
        ...
        // Mutators
        /// set base point
        void set_base_point(const Point &pointemp)
        {
            base_point_ = pointemp;
        }
        /// set bearing
    ...
    public:
        ...
        void push_back(const Point &pointemp)
        {
    ...
    public:
        ...
        /** \brief  offsets a list of simple ccw oriented polygons by a specified
         *          distance delta and then performs a union operation.
         *          Note that the offset distance can be positive (expansion)
         *          or negative (contraction). Corners are roughly mitered, but
         *          guaranteed that all points on the new polygon are at least
         *          delta from the original polygon. The rough mitering operation
         *          uses a threshold angle (45 degrees) above which the expanded
         *          vertex is replaced by two vertices. In the worst case, the
         *          number of vertices in the expanded polygon will be twice that
        ...
        public:
            ...
            bool operator<(const Shortest_Path_Node &spn2) const
            {
                ...
                if (f1 < f2)
                    ...
                    return false;
                else if (&(*(this->parent_search_tree_location)) < &(*(spn2.parent_search_tree_location)))
                    return true;
                else
                    return false;
            }
    ...
    private:
        ...
        struct Polar_Edge
        {
            ...
            Polar_Edge(const Polar_Point &ppoint1,
                       const Polar_Point &ppoint2) : first(ppoint1), second(ppoint2) {}
        };
        ...
        public:
            ...
            bool is_first; // True iff polar_point is the first_point of the
                           ...
                           // incident_edge.
            void set_polar_point(const Polar_Point &ppointemp)
            {
                set_polar_origin(ppointemp.polar_origin());
                set_x(ppointemp.x());
                set_y(ppointemp.y());
                set_range(ppointemp.range());
                set_bearing(ppointemp.bearing());
            }
            ...
            {
                if (Polar_Point(ppwei1) == Polar_Point(ppwei2) and !ppwei1.is_first and ppwei2.is_first)
                    return true;
                else
        ...
        public:
            Incident_Edge_Compare(const Point &observer,
                                  ...
                                  double epsilon_temp) : observer_pointer(&observer),
