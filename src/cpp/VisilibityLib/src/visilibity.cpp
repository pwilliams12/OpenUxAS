...
namespace VisiLibity
{
    ...
    double uniform_random_sample(double lower_bound, double upper_bound)
    {
        ...
        sample_point = lower_bound + span * static_cast<double>(std::rand()) / static_cast<double>(RAND_MAX);
        return sample_point;
    }
    ...
    Point Point::projection_onto(const Line_Segment &line_segment_temp) const
    {
        ...
        // The projection of pointemp onto the line determined by
        ...
        double theta =
            ((line_segment_temp.second().x() - x()) * (line_segment_temp.second().x() - line_segment_temp.first().x()) + (line_segment_temp.second().y() - y()) * (line_segment_temp.second().y() - line_segment_temp.first().y())) / (pow(line_segment_temp.second().x() - line_segment_temp.first().x(), 2) + pow(line_segment_temp.second().y() - line_segment_temp.first().y(), 2));
        // std::cout << "\E[1;37;40m" << "Theta is: " << theta << "\x1b[0m"
        //<< std::endl;
        if ((0.0 <= theta) and (theta <= 1.0))
            return theta * line_segment_temp.first() + (1.0 - theta) * line_segment_temp.second();
        // Else pick closest endpoint.
        if (distance(*this, line_segment_temp.first()) < distance(*this, line_segment_temp.second()))
            return line_segment_temp.first();
    ...
    Point Point::projection_onto(const Polyline &polyline_temp) const
    {
        ...
        {
            pointemp = projection_onto(Line_Segment(polyline_temp[i],
                                                    polyline_temp[i + 1]));
            if (distance(*this, pointemp) < running_min)
            {
                                                 ...
                                                 environment_temp) const
    {
        ...
        Point running_projection = projection_onto_vertices_of(environment_temp.outer_boundary_);
        double running_min = distance(*this, running_projection);
        Point pointemp;
        for (unsigned i = 0; i < environment_temp.h(); i++)
        {
            pointemp = projection_onto_vertices_of(environment_temp.holes_[i]);
            if (distance(*this, pointemp) < running_min)
            {
                running_projection = pointemp;
                running_min = distance(*this, running_projection);
    ...
    Point Point::projection_onto_boundary_of(const Polygon &polygon_temp) const
    {
        ...
        {
            pointemp = projection_onto(Line_Segment(polygon_temp[i],
                                                    polygon_temp[i + 1]));
            if (distance(*this, pointemp) < running_min)
            {
                running_projection = pointemp;
                                                 ...
                                                 environment_temp) const
    {
        ...
        Point running_projection = projection_onto_boundary_of(environment_temp.outer_boundary_);
        double running_min = distance(*this, running_projection);
        Point pointemp;
        for (unsigned i = 0; i < environment_temp.h(); i++)
        {
            pointemp = projection_onto_boundary_of(environment_temp.holes_[i]);
            if (distance(*this, pointemp) < running_min)
            {
                running_projection = pointemp;
                running_min = distance(*this, running_projection);
                               ...
                               double epsilon) const
    {
        ...
        if (distance(*this, projection_onto_boundary_of(environment_temp)) <= epsilon)
        {
            return true;
        }
        return false;
                                        ...
                                        double epsilon) const
    {
        ...
        return in(line_segment_temp, epsilon) and distance(*this, line_segment_temp.first()) > epsilon and distance(*this, line_segment_temp.second()) > epsilon;
    }
                   ...
                   double epsilon) const
    {
        ...
        {
            if ((((polygon_temp[i].y() <= y()) and (y() < polygon_temp[j].y())) or ((polygon_temp[j].y() <= y()) and (y() < polygon_temp[i].y()))) and (x() < (polygon_temp[j].x() - polygon_temp[i].x()) * (y() - polygon_temp[i].y()) / (polygon_temp[j].y() - polygon_temp[i].y()) + polygon_temp[i].x()))
                c = !c;
        }
        return c;
    }
                               ...
                               double epsilon) const
    {
        ...
        if (distance(line_segment_temp.first(), *this) <= epsilon or distance(line_segment_temp.second(), *this) <= epsilon)
            return true;
        return false;
    }

    // note: static function
    Angle Point::angle_separation(const Point &a, const Point &b, const Point &c)
    {
        Point q1(b.x() - a.x(), b.y() - a.y());
        Point q2(c.x() - b.x(), c.y() - b.y());
        return Angle(atan2(q2.y(), q2.x()) - atan2(q1.y(), q1.x()) + M_PI);
    }

    // note: static function
    Point Point::rotate(const Point &p, double th)
    {
                                    ...
                                    double epsilon)
    {
        ...
        Point pointemp(this->projection_onto_vertices_of(polygon_temp));
        if (distance(*this, pointemp) <= epsilon)
            *this = pointemp;
    }
                                    ...
                                    double epsilon)
    {
        ...
        Point pointemp(this->projection_onto_vertices_of(environment_temp));
        if (distance(*this, pointemp) <= epsilon)
            *this = pointemp;
    }
                                    ...
                                    double epsilon)
    {
        ...
        Point pointemp(this->projection_onto_boundary_of(polygon_temp));
        if (distance(*this, pointemp) <= epsilon)
            *this = pointemp;
    }
                                    ...
                                    double epsilon)
    {
        ...
        Point pointemp(this->projection_onto_boundary_of(environment_temp));
        if (distance(*this, pointemp) <= epsilon)
            *this = pointemp;
    }
    ...
    {
        assert(pointemp == pointemp and line_segment_temp.size() > 0);

        return distance(pointemp,
                        pointemp.projection_onto(line_segment_temp));
    }
    ...
    double distance(const Point &pointemp,
                    const Ray &ray_temp)
    {
        assert(pointemp == pointemp and ray_temp == ray_temp);
        return distance(pointemp,
                        pointemp.projection_onto(ray_temp));
    }
    ...
    {
        assert(pointemp == pointemp and polyline_temp.size() > 0);

        double running_min = distance(pointemp, polyline_temp[0]);
        double distance_temp;
        for (unsigned i = 0; i < polyline_temp.size() - 1; i++)
        {
            distance_temp = distance(pointemp, Line_Segment(polyline_temp[i],
                                                            polyline_temp[i + 1]));
    ...
    double distance(const Polyline &polyline_temp,
                    const Point &pointemp)
    {
        return distance(pointemp, polyline_temp);
    }

    double boundary_distance(const Point &pointemp,
                             const Polygon &polygon_temp)
    {
        assert(pointemp == pointemp and polygon_temp.n() > 0);

        double running_min = distance(pointemp, polygon_temp[0]);
        double distance_temp;
        for (unsigned i = 0; i <= polygon_temp.n(); i++)
        {
            distance_temp = distance(pointemp, Line_Segment(polygon_temp[i],
                                                            polygon_temp[i + 1]));
            if (distance_temp < running_min)
                running_min = distance_temp;
        }
    ...
    double boundary_distance(const Polygon &polygon_temp, const Point &pointemp)
    {
        return boundary_distance(pointemp, polygon_temp);
    }

    double boundary_distance(const Point &pointemp,
                             const Environment &environment_temp)
    {
        assert(pointemp == pointemp and environment_temp.n() > 0);

        double running_min = distance(pointemp, environment_temp[0][0]);
        double distance_temp;
        for (unsigned i = 0; i <= environment_temp.h(); i++)
        {
            distance_temp = boundary_distance(pointemp, environment_temp[i]);
            if (distance_temp < running_min)
                running_min = distance_temp;
        }
        return running_min;
    ...
    double boundary_distance(const Environment &environment_temp,
                             const Point &pointemp)
    {
        return boundary_distance(pointemp, environment_temp);
    }

    std::ostream &operator<<(std::ostream &outs, const Point &pointemp)
    {
        outs << pointemp.x() << "  " << pointemp.y();
        return outs;
    }
    ...
    Line_Segment::Line_Segment(const Point &first_pointemp,
    ...
    void Line_Segment::set_first(const Point &pointemp, double epsilon)
    {
    ...
    void Line_Segment::set_second(const Point &pointemp, double epsilon)
    {
    ...
    {
        if (line_segment1.size() != line_segment2.size() or line_segment1.size() == 0 or line_segment2.size() == 0)
            return false;
        else if (line_segment1.first() == line_segment2.first() and line_segment1.second() == line_segment2.second())
            return true;
        else
            return false;
    }
                    ...
                    Line_Segment line_segment2, double epsilon)
    {
        if (line_segment1.size() != line_segment2.size() or line_segment1.size() == 0 or line_segment2.size() == 0)
            return false;
                           ...
                           line_segment2.first()) <= epsilon and
                  distance(line_segment1.second(),
                           line_segment2.second()) <= epsilon) or
                 (distance(line_segment1.first(),
                           line_segment2.second()) <= epsilon and
                  distance(line_segment1.second(),
                          ...
                          const Line_Segment &line_segment2, double epsilon)
    {
        ...
        if (cross(b - a, c - b) * cross(b - a, d - b) < 0 and cross(d - c, b - d) * cross(d - c, a - d) < 0)
            return true;
        return false;
    }
    ...
    Line_Segment intersection_infinite(const Line_Segment &line_segment1, const Line_Segment &line_segment2, double epsilon)
    {
        // Initially empty.
        Line_Segment line_segment_temp;

        if (line_segment1.size() < 2 or line_segment2.size() < 2)
            return line_segment_temp;

        // line 1 formed as a1 x + b1 y + c1 = 0
        double a1 = line_segment1.second().y() - line_segment1.first().y();
        double b1 = line_segment1.first().x() - line_segment1.second().x();
        double c1 = line_segment1.second().x() * line_segment1.first().y() -
                    line_segment1.first().x() * line_segment1.second().y();

        // line 2 formed as a2 x + b2 y + c2 = 0
        double a2 = line_segment2.second().y() - line_segment2.first().y();
        double b2 = line_segment2.first().x() - line_segment2.second().x();
        double c2 = line_segment2.second().x() * line_segment2.first().y() -
                    line_segment2.first().x() * line_segment2.second().y();

        double denom = a1 * b2 - a2 * b1;
        if (fabs(denom) < 1e-12 || fabs(denom) < fabs(epsilon))
            return line_segment_temp;

        line_segment_temp.set_first(Point((b1 * c2 - b2 * c1) / denom, (a2 * c1 - a1 * c2) / denom), epsilon);
        return line_segment_temp;
    }
    ...
    Polar_Point::Polar_Point(const Point &polar_origin_temp,
                             const Point &pointemp,
                             double epsilon) : Point(pointemp)
    {
        polar_origin_ = polar_origin_temp;
        if (polar_origin_ == polar_origin_ and pointemp == pointemp and distance(polar_origin_, pointemp) <= epsilon)
        {
            bearing_ = Angle(0.0);
            range_ = 0.0;
        }
        else if (polar_origin_ == polar_origin_ and pointemp == pointemp)
        {
            bearing_ = Angle(pointemp.y() - polar_origin_temp.y(),
                             pointemp.x() - polar_origin_temp.x());
            range_ = distance(polar_origin_temp, pointemp);
        }
    }
    ...
    {
        if (polar_point1.polar_origin() == polar_point2.polar_origin() and polar_point1.range() == polar_point2.range() and polar_point1.bearing() == polar_point2.bearing())
            return true;
        return false;
    }
    bool operator!=(const Polar_Point &polar_point1,
    ...
    bool operator>(const Polar_Point &polar_point1,
                   const Polar_Point &polar_point2)
    {
        if (polar_point1.polar_origin() != polar_point1.polar_origin() or polar_point1.range() != polar_point1.range() or polar_point1.bearing() != polar_point1.bearing() or polar_point2.polar_origin() != polar_point2.polar_origin() or polar_point2.range() != polar_point2.range() or polar_point2.bearing() != polar_point2.bearing())
            return false;

        if (polar_point1.bearing() > polar_point2.bearing())
            return true;
        else if (polar_point1.bearing() == polar_point2.bearing() and polar_point1.range() > polar_point2.range())
            return true;
        return false;
    }
    bool operator<(const Polar_Point &polar_point1,
    ...
    std::ostream &operator<<(std::ostream &outs,
                             const Polar_Point &polar_pointemp)
    {
        outs << polar_pointemp.bearing() << "  " << polar_pointemp.range();
        return outs;
    }

    // Ray

    Ray::Ray(Point base_pointemp, Point bearing_point)
    {
        assert(!(base_pointemp == bearing_point));

        base_point_ = base_pointemp;
        bearing_ = Angle(bearing_point.y() - base_pointemp.y(),
                         bearing_point.x() - base_pointemp.x());
    }

    bool operator==(const Ray &ray1,
                    const Ray &ray2)
    {
        if (ray1.base_point() == ray2.base_point() and ray1.bearing() == ray2.bearing())
            return true;
        else
            return false;
    }
                              ...
                              double epsilon)
    {
        ...
        double R = distance(ray_temp.base_point(), line_segment_temp) + line_segment_temp.length();
        Line_Segment seg_approx =
            Line_Segment(ray_temp.base_point(), ray_temp.base_point() +
                                                    Point(R * std::cos(ray_temp.bearing().get()),
                                                          R * std::sin(ray_temp.bearing().get())));
        ...
        if (intersect_seg.size() == 2 and distance(intersect_seg.first(), ray_temp.base_point()) >
    ...
    Polygon::Polygon(const std::string &filename)
    {
        ...
        {
            pointemp.set_x(x_temp);
            pointemp.set_y(y_temp);
            vertices_.push_back(pointemp);
        }
        fin.close();
    }
    ...
    unsigned Polygon::r() const
    {
        ...
        if (vertices_.size() > 1)
        {
            ...
            for (unsigned i = 0; i <= n() - 1; i++)
                if (((*this)[i + 1].x() - (*this)[i].x()) * ((*this)[i + 2].y() - (*this)[i].y()) - ((*this)[i + 1].y() - (*this)[i].y()) * ((*this)[i + 2].x() - (*this)[i].x()) < 0)
                    r_count++;
            if (area() < 0)
            {
                r_count = n() - r_count;
    ...
    bool Polygon::is_simple(double epsilon) const
    {
        ...
        for (unsigned i = 0; i < n() - 2; i++)
            for (unsigned j = i + 2; j <= n() - 1; j++)
                if (0 != (j + 1) % vertices_.size() and distance(Line_Segment((*this)[i], (*this)[i + 1]),
    ...
    double Polygon::area() const
    {
        ...
        if (n() == 0)
            return 0;
        for (unsigned i = 0; i <= n() - 1; i++)
            area_temp += (*this)[i].x() * (*this)[i + 1].y() - (*this)[i + 1].x() * (*this)[i].y();
    ...
    Point Polygon::centroid() const
    {
        ...
        double x_temp = 0;
        for (unsigned i = 0; i <= n() - 1; i++)
            x_temp += ((*this)[i].x() + (*this)[i + 1].x()) * ((*this)[i].x() * (*this)[i + 1].y() - (*this)[i + 1].x() * (*this)[i].y());
        double y_temp = 0;
        for (unsigned i = 0; i <= n() - 1; i++)
            y_temp += ((*this)[i].y() + (*this)[i + 1].y()) * ((*this)[i].x() * (*this)[i + 1].y() - (*this)[i + 1].x() * (*this)[i].y());
    ...
    bool Polygon::offset_polygons(std::vector<Polygon> &polygonList, std::vector<Polygon> &resultingPolygons, double delta, double epsilon)
    {
        if (polygonList.empty())
            return true;
        std::vector<double> deltaList(polygonList.size(), delta);
        return offset_polygons(polygonList, resultingPolygons, deltaList, epsilon);
    }

    bool Polygon::offset_polygons(std::vector<Polygon> &polygonList, std::vector<Polygon> &resultingPolygons, std::vector<double> &delta, double epsilon)
    {
        unsigned int n, k, N;
        epsilon = fabs(epsilon); // only makes sense as a positive value

        if (polygonList.empty())
            return true;
        if (polygonList.size() != delta.size())
            return false;
        int deltaValue = 0;
        if (delta[0] < -epsilon)
            deltaValue = -1;
        ...
        for (n = 1; n < delta.size(); n++)
        {
            ...
            {
                if (nValue != deltaValue)
                    return false;
            }
        }

        // determine how to miter the corners - if the angle is greater
        // than the threshold, miter with two vertices by forming a line
        // tangent to the miter circle around the vertex in question
        double angleThreshold = 45.0 / 180.0 * M_PI;

        // if the offset is within numerical tolerance, return merged input list
        if (deltaValue == 0)
        {
        ...
        std::vector<Polygon> toMerge;
        // to ensure overlap, add a small bit to the end of each
        // rectangular edge increase
        double fuzz = fabs(epsilon) / 2.0;
        if (fuzz < 1e-11)
            fuzz = 1e-11;

        // build expansion pieces around every polygon edge: this consists of
        // (1) a rectangle that matches the length of the edge but has 2*delta width
        // (2) a corner piece that fills in to the intersection of the adjacent
        //     edges or miters the angle with an extra segment
        for (k = 0; k < polygonList.size(); k++)
        {
            // add in the original polygon to ensure no holes for expansion
            if (deltaValue > 0)
                toMerge.push_back(polygonList[k]);

            if (fabs(delta[k]) < epsilon)
                continue;
            ...
            for (n = 0; n < N; n++)
            {
                ...
                Point a = polygonList[k][n];
                Point b = polygonList[k][(n + 1) % N];
                Point c = polygonList[k][(n + 2) % N];
                double beta = Point::angle_separation(a, b, c).get();

                // build the slightly increased rectangle around the edge
                Point p1 = Point::normalize(Point::rotate(c - b, -M_PI / 2.0)) * fabs(delta[k]) + b;
                newPoints.push_back(p1 + Point::normalize(b - c) * fuzz);
                Point p2 = Point::normalize(Point::rotate(c - b, -M_PI / 2.0)) * fabs(delta[k]) + c;
                newPoints.push_back(p2 + Point::normalize(c - b) * fuzz);
                Point p3 = Point::normalize(Point::rotate(c - b, M_PI / 2.0)) * fabs(delta[k]) + c;
                newPoints.push_back(p3 + Point::normalize(c - b) * fuzz);
                Point p4 = Point::normalize(Point::rotate(c - b, M_PI / 2.0)) * fabs(delta[k]) + b;
                newPoints.push_back(p4 + Point::normalize(b - c) * fuzz);
                toMerge.push_back(Polygon(newPoints));

                // calculate points corresponding to preceeding edge
                Point p5 = Point::normalize(Point::rotate(b - a, -M_PI / 2.0)) * fabs(delta[k]) + b;
                Point p6 = Point::normalize(Point::rotate(b - a, -M_PI / 2.0)) * fabs(delta[k]) + a;
                Point p7 = Point::normalize(Point::rotate(b - a, M_PI / 2.0)) * fabs(delta[k]) + a;
                Point p8 = Point::normalize(Point::rotate(b - a, M_PI / 2.0)) * fabs(delta[k]) + b;

                // build corner piece, reflex vertices need the piece on the opposite side
                // from non-reflex vertices
                newPoints.clear();
                if (beta < M_PI) // reflex vertex
                {
                    double gamma = Point::angle_separation(p4, b, p8).get();
                    Line_Segment xing = intersection_infinite(Line_Segment(p3, p4, epsilon), Line_Segment(p7, p8, epsilon), epsilon);
                    if (xing.size() > 0)
                    {
                        if (gamma > angleThreshold) // rough miter
                        {
                            newPoints.push_back(b);
                            newPoints.push_back(p4);
                            double extendDist = tan(gamma / 4.0) * fabs(delta[k]);
                            newPoints.push_back(Point::normalize(xing.first() - p4) * extendDist + p4);
                            newPoints.push_back(Point::normalize(xing.first() - p8) * extendDist + p8);
                            newPoints.push_back(p8);
                        }
                        else // simple edge intersection
                        {
                            newPoints.push_back(b);
                            newPoints.push_back(p4);
                            newPoints.push_back(xing.first());
                            newPoints.push_back(p8);
                        }
                    }
                    else // triangle
                ...
                {
                    double gamma = Point::angle_separation(p5, b, p1).get();
                    Line_Segment xing = intersection_infinite(Line_Segment(p2, p1, epsilon), Line_Segment(p6, p5, epsilon), epsilon);
                    if (xing.size() > 0)
                    {
                        if (gamma > angleThreshold) // rough miter
                        {
                            ...
                            double extendDist = tan(gamma / 4.0) * fabs(delta[k]);
                            newPoints.push_back(Point::normalize(xing.first() - p5) * extendDist + p5);
                            newPoints.push_back(Point::normalize(xing.first() - p1) * extendDist + p1);
        ...
        // if contraction, only return holes
        if (deltaValue > 0)
        {
            for (n = 0; n < newPolygons.size(); n++)
                if (newPolygons[n].area() > fabs(epsilon))
                    resultingPolygons.push_back(newPolygons[n]);
        }
        else
        {
            // merge back in polygons that were not shrunk
            std::vector<VisiLibity::Polygon> finalMerge;
            for (k = 0; k < polygonList.size(); k++)
            {
                if (fabs(delta[k]) < epsilon)
                    finalMerge.push_back(polygonList[k]);
            }

            if (finalMerge.size() > 0)
            {
                for (n = 0; n < newPolygons.size(); n++)
                    if (newPolygons[n].area() < -fabs(epsilon))
                    {
                        newPolygons[n].reverse();
    ...
    void Polygon::eliminate_redundant_vertices(double epsilon)
    {
        ...
        while (third <= vertices_.size())
        {
            line = Line_Segment((*this)[first], (*this)[third], epsilon);
            if (line.size() != 2)
            { // first and third are the same point
                // there is a spur to the second
                first = third;
                second = third + 1;
                third = third + 2;
                continue;
            ...
            if (distance(line, (*this)[second]) <= epsilon)
            {
                //=>skip it
                second = third;
                third++;
        ...
        if (vertices_temp.size() > 0 && distance(Line_Segment(vertices_temp.front(),
                                                              vertices_temp.back()),
    ...
    {
        if (outer_boundary_.is_in_standard_form() == false or outer_boundary_.area() < 0)
            return false;
                                        ...
                                        double epsilon)
    {
        ...
        while (!Q.empty())
        {
            ...
            else
            {
                ...
                for (unsigned i = 0; i < n(); i++)
                {
                    if (current_node.vertex_index != i)
                        if (visibility_graph(current_node.vertex_index, i))
                        {
                            child.vertex_index = i;
                            child.parent_search_tree_location = current_node.search_tree_location;
                            child.cost_to_come = current_node.cost_to_come + distance((*this)(current_node.vertex_index),
                                                                                      (*this)(i));
                            child.estimated_cost_to_go = distance((*this)(i), finish);
                            children.push_back(child);
                ...
                if (finish_visible[current_node.vertex_index])
                {
                    ...
                    child.parent_search_tree_location = current_node.search_tree_location;
                    child.cost_to_come = current_node.cost_to_come + distance((*this)(current_node.vertex_index), finish);
                 ...
                 children_itr++)
            {
                         ...
                         children_itr->cost_to_come)
                {
                    ...
                    children_itr->search_tree_location->parent_search_tree_location = children_itr->parent_search_tree_location;
        ...
        if (solution_found)
        {
            ...
            while (true)
            {
                ...
                if (shortest_path_output.size() > 0 and distance(shortest_path_output[shortest_path_output.size() - 1],
                                                                 waypoint) > epsilon)
    ...
    std::pair<unsigned, unsigned> Environment::one_to_two(unsigned k) const
    {
        ...
        while (k >= vertex_count_up_to_current_polygon and current_polygon_index < (*this).h())
        {
                                      ...
                                      double epsilon) const
    {
        ...
        return (
            ...
            distance(observer, point1) > epsilon and distance(observer, point2) > epsilon and distance(observer, point3) > epsilon
            // Test whether there is a spike with point2 as the tip
            and ((distance(observer, point2) >= distance(observer, point1) and distance(observer, point2) >= distance(observer, point3)) or (distance(observer, point2) <= distance(observer, point1) and distance(observer, point2) <= distance(observer, point3)))
            // and the pike is sufficiently sharp,
            and distance(Ray(observer, point1), point2) <= epsilon and distance(Ray(observer, point3), point2) <= epsilon);
        // Formerly used
                                                 ...
                                                 double epsilon)
    {
        ...
        while (vertices_.size() >= 3 and is_spike(observer,
                                         ...
                                         double epsilon)
    {
        ...
        for (unsigned i = 0; i < vertices_.size(); i++)
            if (distance((*this)[i + 2],
                         Line_Segment((*this)[i], (*this)[i + 1])) <= epsilon or
                distance((*this)[i],
        ...
        : observer_(observer)
    {
            ...
            for (unsigned j = 0; j < environment_temp[i].n(); j++)
            {
                ...
                else if (std::fabs(ppoint1.y() - ppoint2.y()) > epsilon)
                {
                        ...
                        observer.x() < t * ppoint1.x() + (1 - t) * ppoint2.x())
                    {
                        ...
                        split_bottom = split_top = Polar_Point(observer,
                    ...
                    else if (ppoint1.bearing() >= ppoint2.bearing() and ppoint2.bearing() == Angle(0.0) and ppoint1.bearing() > Angle(M_PI))
                        ppoint2.set_bearing_to_2pi();
                    // Filter out edges which run 'against the grain'.
                    else if ((ppoint1.bearing() == Angle(0, 0) and ppoint2.bearing() > Angle(M_PI)) or ppoint1.bearing() >= ppoint2.bearing())
                        continue;
        ...
        // observer.
        if (distance(observer, active_edge->first) > epsilon and distance(observer, active_edge->second) > epsilon)
        {
        ...
        while (!q1.empty())
        {
            ...
            if (current_vertex.incident_edge == active_edge and !current_vertex.is_first)
            {
            ...
            if (current_vertex.is_first)
            {
                ...
                xing = intersection(Ray(observer, current_vertex.bearing()),
                                    Line_Segment(active_edge->first,
                                                 active_edge->second),
                                    epsilon);
                if (xing.size() == 0 or (distance(active_edge->first, observer) <= epsilon and active_edge->second.bearing() <= current_vertex.bearing()) or active_edge->second < current_vertex)
                {
                    k_range = INFINITY;
                }
                else
                ...
                // Insert e into q2 as long as it doesn't contain the
                // observer.
                if (distance(observer, e->first) > epsilon and distance(observer, e->second) > epsilon)
                {
                ...
                if (k_range >= current_vertex.range())
                {
                    ...
                    if (xing.size() > 0
                        // and k == k
                        and k_range != INFINITY and distance(k, current_vertex) > epsilon and distance(active_edge->first, observer) > epsilon)
                    {
    ...
    Visibility_Graph &Visibility_Graph::operator=(const Visibility_Graph &visibility_graph_temp)
    {
    ...
    boost_polygon to_boost(std::vector<Polygon> visPolyList)
    {
        ...
        if (!boost::geometry::is_valid(b_poly, failure))
        {
            ...
            b_poly.clear();
            std::cerr << "Attempted to merge invalid polygon" << std::endl;
        }
    ...
    void snap_near(std::vector<boost_polygon> &polygonList, double epsilon)
    {
                        ...
                        boost::geometry::distance(polygonList[idx1], polygonList[idx2]) < epsilon)
                    {
                        ...
                        for (int pointIdx = 0; pointIdx < poly1[0].n(); pointIdx++)
                        {
                            if (boundary_distance(poly1[0][pointIdx], poly2) < epsilon)
                            {
                                ...
                                // only moves point if distance to boundary is less than epsilon
                                poly1[0][pointIdx].snap_to_boundary_of(poly2, epsilon);
                            }
                        }
                        ...
                        for (int pointIdx = 0; pointIdx < poly2[0].n(); pointIdx++)
                        {
                            if (boundary_distance(poly2[0][pointIdx], poly1) < epsilon)
                            {
                                // attempt to snap point to boundary of poly2
                                // only moves point if distance to boundary is less than epsilon
                                poly2[0][pointIdx].snap_to_boundary_of(poly1, epsilon);
                            }
                        }
                        polygonList[idx1] = to_boost(poly1);
                        polygonList[idx2] = to_boost(poly2);
                    }
                }
            }
        }
    ...
    bool Polygon::boost_union(std::vector<Polygon> &polygonList, std::vector<Polygon> &resultingPolygons, double epsilon)
    {
        ...
        while (!overlapping.empty() && overlapping != tangent)
        {
            ...
            for (auto revIt = overlapping.rbegin(); revIt != overlapping.rend(); revIt++)
            {
                ...
                {
                    // unable to merge polygons, most likely because they share a vertex are not overlapping or adjacent
